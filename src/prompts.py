...
- verify_and_fix_exam_question_tool：一键核查和修正考题。该工具自动执行核查和修正流程：先核查考题，如果不合规则进行修正，最多修正{max_fix_attempts}次；如果题目合规，则直接返回考题信息。
...
- 工具 verify_and_fix_exam_question_tool 已内置重试逻辑（最多{max_fix_attempts}次），因此无需在规划中额外处理重试。
- 保持响应简洁专业，专注于任务规划与执行。
'''
